// data/workouts.js
// Lista genérica de treinos focada em Calistenia Asiática (21 dias)

export const weeks = [
  {
    "id": 1,
    "title": "Semana 1",
    "subtitle": "Adaptação",
    "description": "Aprender os movimentos e criar o hábito",
    "days": [
      {
        "id": 1,
        "title": "Dia 1",
        "duration": "15 min",
        "exercisesCount": 10,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "30s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Torção Rítmica em Pé",
            "reps": "30s",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "30s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Flexão na Parede",
            "reps": "30s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede, excelente para iniciantes desenvolverem força no peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          },
          {
            "name": "Prancha Lotus",
            "reps": "30s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "30s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "30s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "30s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Lado Direito",
            "reps": "30s",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ]
          },
          {
            "name": "Afundo Crescente",
            "reps": "30s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com alongamento crescente para flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          }
        ]
      },
      {
        "id": 2,
        "title": "Dia 2",
        "duration": "15 min",
        "exercisesCount": 10,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Torção Rítmica em Pé",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Crescente",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com alongamento crescente para flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Crescente",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com alongamento crescente para flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Prancha Lotus",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ],
            "reps": "30s"
          }
        ]
      },
      {
        "id": 3,
        "title": "Dia 3",
        "duration": "15 min",
        "exercisesCount": 10,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Torção Rítmica em Pé",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Flexão na Parede",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede, excelente para iniciantes desenvolverem força no peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Prancha Lotus",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ],
            "reps": "30s"
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          }
        ]
      },
      {
        "id": 4,
        "title": "Dia 4",
        "duration": "15 min",
        "exercisesCount": 10,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Torção Rítmica em Pé",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Flexão na Parede",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede, excelente para iniciantes desenvolverem força no peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação de Perna Bambu",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Flexão na Parede",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede, excelente para iniciantes desenvolverem força no peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação de Perna Bambu",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Crescente",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com alongamento crescente para flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          }
        ]
      },
      {
        "id": 5,
        "title": "Dia 5",
        "duration": "15 min",
        "exercisesCount": 10,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Torção Rítmica em Pé",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Crescente",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com alongamento crescente para flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Crescente",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com alongamento crescente para flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação de Perna Bambu",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ],
            "reps": "30s"
          }
        ]
      },
      {
        "id": 6,
        "title": "Dia 6",
        "duration": "15 min",
        "exercisesCount": 10,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Torção Rítmica em Pé",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação de Perna Bambu",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Prancha Lotus",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ],
            "reps": "30s"
          },
          {
            "name": "Agachamento Templo",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação de Perna Bambu",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ],
            "reps": "30s"
          },
          {
            "name": "Afundo Lado Direito",
            "video": "/entregravel/videos/Afundo_Lado_Direito.mp4",
            "description": "Passada focada no lado direito, melhorando simetria e força nas pernas.",
            "tags": [
              "Pernas",
              "Equilíbrio"
            ],
            "reps": "30s"
          }
        ]
      },
      {
        "id": 7,
        "title": "Dia 7",
        "duration": "10 min",
        "exercisesCount": 5,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Torção Rítmica em Pé",
            "video": "/entregravel/videos/Torcao_Ritmica_em_Pe.mp4",
            "description": "Rotação suave do tronco em pé, ativando a cintura e melhorando a mobilidade.",
            "tags": [
              "Oblíquos",
              "Core"
            ],
            "reps": "30s"
          },
          {
            "name": "Prancha Lotus",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ],
            "reps": "30s"
          },
          {
            "name": "Flexão na Parede",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede, excelente para iniciantes desenvolverem força no peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ],
            "reps": "30s"
          },
          {
            "name": "Elevação Pélvica Fluida",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ],
            "reps": "30s"
          }
        ]
      }
    ]
  },
  {
    "id": 2,
    "title": "Semana 2",
    "subtitle": "Ativação",
    "description": "Intensificando o ritmo e ganhando força",
    "days": [
      {
        "id": 8,
        "title": "Dia 8",
        "duration": "20 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 9,
        "title": "Dia 9",
        "duration": "20 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 10,
        "title": "Dia 10",
        "duration": "20 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 11,
        "title": "Dia 11",
        "duration": "20 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 12,
        "title": "Dia 12",
        "duration": "20 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 13,
        "title": "Dia 13",
        "duration": "20 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 14,
        "title": "Dia 14",
        "duration": "10 min",
        "exercisesCount": 11,
        "exercises": [
          {
            "name": "Círculos de Braço Garça",
            "reps": "40s",
            "video": "/entregravel/videos/Circulos_de_Braco_Garca.mp4",
            "description": "Movimentos circulares suaves dos braços, aquecendo ombros e melhorando mobilidade.",
            "tags": [
              "Ombros",
              "Braços"
            ]
          },
          {
            "name": "Polichinelo Leque",
            "reps": "40s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "40s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "40s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "40s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "40s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "40s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Chute Traseiro Rítmico",
            "reps": "40s",
            "video": "/entregravel/videos/chuteTraseiro.mp4",
            "description": "Chutes para trás alternados, fortalecendo glúteos e melhorando mobilidade.",
            "tags": [
              "Glúteos",
              "Cardio"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "40s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      }
    ]
  },
  {
    "id": 3,
    "title": "Semana 3",
    "subtitle": "Intensificação",
    "description": "Resultados visíveis e domínio corporal",
    "days": [
      {
        "id": 15,
        "title": "Dia 15",
        "duration": "25 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 16,
        "title": "Dia 16",
        "duration": "25 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 17,
        "title": "Dia 17",
        "duration": "25 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 18,
        "title": "Dia 18",
        "duration": "25 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 19,
        "title": "Dia 19",
        "duration": "25 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 20,
        "title": "Dia 20",
        "duration": "25 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      },
      {
        "id": 21,
        "title": "Dia 21",
        "duration": "10 min",
        "exercisesCount": 12,
        "exercises": [
          {
            "name": "Polichinelo Leque",
            "reps": "45s",
            "video": "/entregravel/videos/polichinelo.mp4",
            "description": "Pulos suaves com movimentos amplos dos braços, ativando circulação.",
            "tags": [
              "Cardio",
              "Corpo Todo"
            ]
          },
          {
            "name": "Agachamento Templo",
            "reps": "45s",
            "video": "/entregravel/videos/Agachamento_Templo.mp4",
            "description": "Agachamento profundo focando no equilíbrio e no fortalecimento das pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Afundo Crescente (Esq)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna esquerda à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Afundo Crescente (Dir)",
            "reps": "45s",
            "video": "/entregravel/videos/Afundo_Crescente.mp4",
            "description": "Afundo mantido em isometria com a perna direita à frente, focando na flexibilidade.",
            "tags": [
              "Flexibilidade",
              "Pernas"
            ]
          },
          {
            "name": "Agachamento Lateral Deslizante",
            "reps": "45s",
            "video": "/entregravel/videos/agachamentoLateral.mp4",
            "description": "Deslocamento lateral em posição de agachamento, focando em pernas e glúteos.",
            "tags": [
              "Pernas",
              "Glúteos"
            ]
          },
          {
            "name": "Joelhos Altos Leves",
            "reps": "45s",
            "video": "/entregravel/videos/joelhoAlto.mp4",
            "description": "Elevação alternada dos joelhos de forma leve e rítmica, ativando cardio.",
            "tags": [
              "Cardio",
              "Pernas"
            ]
          },
          {
            "name": "Abdominal Bicicleta Fluido",
            "reps": "45s",
            "video": "/entregravel/videos/Abdominal_Bicicleta_Fluido.mp4",
            "description": "Movimento contínuo de bicicleta no chão, ativando o abdômen e oblíquos.",
            "tags": [
              "Oblíquos",
              "Core"
            ]
          },
          {
            "name": "Elevação de Perna Bambu",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_de_Perna_Bambu.mp4",
            "description": "Elevação de pernas alternada, focada em ativar a parte inferior do abdômen.",
            "tags": [
              "Abdômen",
              "Pernas"
            ]
          },
          {
            "name": "Prancha Lótus",
            "reps": "45s",
            "video": "/entregravel/videos/Prancha_Lotus.mp4",
            "description": "Prancha com foco na respiração e sustentação, fortalecendo todo o core abdominal.",
            "tags": [
              "Core",
              "Resistência"
            ]
          },
          {
            "name": "Elevação Pélvica Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Elevacao_Pelvica_Fluida.mp4",
            "description": "Elevação do quadril ativando glúteos e posterior de coxa de forma fluida.",
            "tags": [
              "Glúteos",
              "Pernas"
            ]
          },
          {
            "name": "Tríceps no Banco/Cadeira",
            "reps": "45s",
            "video": "/entregravel/videos/tricepsCadeira.mp4",
            "description": "Flexão de braços apoiando em uma cadeira ou banco, isolando o tríceps.",
            "tags": [
              "Braços",
              "Tríceps"
            ]
          },
          {
            "name": "Flexão de Parede Fluida",
            "reps": "45s",
            "video": "/entregravel/videos/Flexao_de_Parede.mp4",
            "description": "Flexão apoiada na parede com movimento constante, desenvolvendo força peitoral.",
            "tags": [
              "Peitoral",
              "Braços"
            ]
          }
        ]
      }
    ]
  }
];
